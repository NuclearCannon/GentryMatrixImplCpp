...
RaderNTTer64::RaderNTTer64(u64 p, u64 q, u64 eta):
    p_(p), g_(findPrimitiveRoot(p)), eta_(eta), q_(q), pinv_(mod_inv(p, q)), 
    gpp(p), gnp(p), b1ntt(p-1), b2ntt(p-1)
{
    ...
    // 计算eta^{g^i}
    vec64 b1(p-1), b2(p-1);
    u64 ieta = mod_inv(eta, q);
    for(int i=0;i<p-1;i++)
    {
        b1[i] = mod_pow(eta,  gpp[i], q);
