...
ComplexMatrixGroup ComplexMatrixGroup::random(long double abs_max, size_t n, size_t p)
{
    size_t len = (p-1)*n*n;
    std::vector<complex> result(len);
    for(size_t i=0; i<len; i++)
    {
        long double abs = random_generators::random_real() * abs_max;
        long double angle = random_generators::random_real() * M_PI * 2;
...
ComplexMatrixGroup ComplexMatrixGroup::random_imag(long double abs_max, size_t n, size_t p)
{
    size_t len = (p-1)*n*n;
    std::vector<complex> result(len);
    for(size_t i=0; i<len; i++)
    {
        long double x = (2*random_generators::random_real()-1) * abs_max;
