...
Plaintext Plaintext::conj(const GentryPolyCtx& ctx) const
{
    return Plaintext(std::make_unique<GentryPoly>(data_->conj().w_inv().xy_inv()));
    // return Plaintext(std::make_unique<GentryPoly>(data_->w_inv().conj()));
}
