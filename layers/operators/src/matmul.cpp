...
public:
    MatmulContext(int n, uint64_t q);
    ~MatmulContext();
    void circledast_u64(uint64_t* dst, const uint64_t* A, const uint64_t* B, size_t n, size_t p);
};

MatmulContext::MatmulContext(int n, uint64_t q):
    n_(n)
{
    fmpz_init_set_ui(q_, q);
...
{
    MatmulContext mc(n, q);
    mc.circledast_u64(C, A, B, n, p);
}
